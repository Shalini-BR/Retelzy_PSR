...
public class Getlatestfilenametest2 {
		     ...
		     while(itr.hasNext())
		     {
		      ...
		      XSSFSheet sheet3 = workbook3.getSheet("hiddendata");
		 	  ...
		 	  for (int j=0;j<totalRows;j++) 
		 	  {
				  ...
				  String firstCellValue = format.formatCellValue(sheet1.getRow(j).getCell(1));
		
		 	     // System.out.println(" first Cell Value"+firstCellValue);
		 	
		 	      if(firstCellValue!=null && !firstCellValue.equalsIgnoreCase("")){
		 	    	  
		 	    	  header = format.formatCellValue(sheet3.getRow(0).getCell(Integer.parseInt(firstCellValue)));
		 	    	  value = format.formatCellValue(sheet3.getRow(1).getCell(Integer.parseInt(firstCellValue)));
		 	    	 System.out.println("Export Excel value: " + header + " = " + value);
		 	    	
				 	excel_data.put(value, header);
...
private static void runCopyPaste(WebDriver driver, String jqxGridId, String string, Object object, String header)
	...
	try {
		...
		WebElement toastmessage = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@id='toastr-container']/div/div/span")));
		System.out.println("toastmessage is " + toastmessage.getText());
		
		WebElement Gridvaluetest = wait.until(ExpectedConditions. presenceOfElementLocated(By.xpath("//*[@id='row0" + jqxGridId + "']/div[5]/div")));
		System.out.println(Gridvaluetest.getText());
        ...
        while (true)
        {
            try
            {
            	WebElement Gridvalue = wait.until(ExpectedConditions.presenceOfElementLocated(
        				By.xpath("//*[@id='row" + i + jqxGridId + "']/div[" + string + "]")));
